...
router.get("/", async (req: Request, res: Response) => {
    try {
        const { sportType, endDate, search } = req.query;
        ...
        // endDate filtresi (Belirli bir tarihten küçük olan abonelikleri getir)
        if (endDate) {
            query.endDate = { $lte: new Date(endDate as string) };
        }

        // Arama parametresi varsa sportType veya endDate alanında arama yap
        if (search) {
            query.$or = [
        ...
        // Sorguyu çalıştır ve startDate'e göre sırala (ascending)
