...
class APIS {
  ...
  final String _baseAPI =
      "http://localhost:5002"; // change it real ip address according to device of local network
  ...
  String memberFiles({required String memberId}) =>
      "$_baseAPI/download/member/$memberId";
  String downloadFile({required String fileId}) => "$_baseAPI/download/$fileId";

  String week({int page = 1, int limit = 10, String? search}) =>
      "$_baseAPI/week?page=${page.toString()}&limit=${limit.toString()}&search=$search";
  ...
  String subscription({int page = 1, int limit = 300, String? search}) {
    String url =
        "$_baseAPI/subscription?page=${page.toString()}&limit=${limit.toString()}";
    if (search != null) url += "&search=$search";
    return url;
  }

  String subscriptionId({required String memberId}) =>
      "$_baseAPI/subscription/$memberId";
...
class APIService<T extends JsonProtocol> {
  ...
  Future<BaseResponseModel<T>> get(
    ...
    try {
      ...
      return BaseResponseModel<T>.fromJson(
          map: jsonDecode(res.body), fromJsonT: fromJsonT);
  ...
  Future<BaseResponseModel<T?>> post(T model,
      {T Function(dynamic json)? fromJsonT,
      String username = "",
      String password = ""}) async {
    String basicAuth =
  ...
  Future<BaseResponseModel<T?>> put(T model,
      {T Function(dynamic json)? fromJsonT,
      String username = "",
      String password = ""}) async {
    String basicAuth =
  ...
  Future<BaseResponseModel<T?>> delete(T model,
      {T Function(dynamic json)? fromJsonT,
      String username = "",
      String password = ""}) async {
    String basicAuth =
  ...
  Future<Map<String, dynamic>> downloadFile({
    ...
    try {
      ...
      if (streamedResponse.statusCode >= 400) {
        throw APIError(Errors.invalidResponseStatus,
            "Dosya indirme başarısız. HTTP kodu: ${streamedResponse.statusCode}");
      }
      ...
      final fileName = _extractFileNameFromHeaders(streamedResponse.headers) ??
          'downloaded_file';

      debugPrint(
          "Dosya indiriliyor: $fileName, Boyut: ${(contentLength / 1024).toStringAsFixed(2)} KB, Tip: $contentType");
  ...
  Future<BaseResponseModel<T?>> uploadFile(
    ...
    try {
      ...
      debugPrint(
          "Yüklenecek dosya: $filePath, Boyut: ${fileSizeMB.toStringAsFixed(2)}MB");
  ...
  factory BaseResponseModel.fromJson(
    ...
    try {
      ...
      return BaseResponseModel(
          success: map["success"], message: map["message"], data: data);
    } catch (err) {
      debugPrint(err.toString());
      rethrow;
    }
